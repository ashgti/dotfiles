...
syn keyword pirOp isle islt iseq isne and not or xor end noop
syn keyword pirOp check_events check_events__ wrapper__ load_bytecode
syn keyword pirOp load_language branch local_branch local_return jump
syn keyword pirOp enternative if unless invokecc invoke yield tailcall
syn keyword pirOp returncc capture_lex newclosure set_args get_params
syn keyword pirOp set_returns get_results set_result_info result_info
syn keyword pirOp set_addr get_addr schedule addhandler push_eh pop_eh
syn keyword pirOp throw rethrow count_eh die exit debug bounds profile
...
syn keyword pirOp getline getfile trap set_label get_label fetch vivify
syn keyword pirOp new root_new find_codepoint unroll close fdopen
syn keyword pirOp getstdin getstdout getstderr setstdin setstdout
syn keyword pirOp setstderr open print say printerr read readline peek
syn keyword pirOp stat seek tell abs add cmod dec div fdiv ceil floor inc
syn keyword pirOp mod mul neg pow sub sqrt acos asec asin atan cos cosh
syn keyword pirOp exp ln log10 log2 sec sech sin sinh tan tanh gcd lcm
syn keyword pirOp fact callmethodcc callmethod tailcallmethod addmethod
syn keyword pirOp can does isa newclass subclass get_class class
syn keyword pirOp addparent removeparent addrole addattribute
...
syn keyword pirOp replace index sprintf stringinfo upcase downcase
...
syn keyword pirOp decodetime decodelocaltime sysinfo sleep store_lex
